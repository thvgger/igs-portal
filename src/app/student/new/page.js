                    ...
                    <div className="block sm:hidden">
                        {studentOutstandingFees.map((fee) => (
                            <div key={fee.id} className="bg-white border border-slate-200 rounded-lg p-4 mb-4 shadow-sm last:mb-0">
                                ...
                                <p className="text-sm text-slate-700"><strong>Term:</strong> {getTermName(fee.termId)}</p>
                                <p className="text-sm text-slate-700 mt-2"><strong>Status:</strong> <span className={`rounded-full px-2 py-0.5 font-bold capitalize text-xs ${statusClass(fee.status)}`}>{fee.status}</span></p>
                            </div>
                        ))}
                    </div>
                )}
