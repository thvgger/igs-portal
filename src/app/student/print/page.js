                    ...
                    <div className="block sm:hidden">
                        {studentPaymentHistory.map((payment) => (
                            <div key={payment.id} className="bg-white border border-slate-200 rounded-lg p-4 mb-4 shadow-sm last:mb-0">
                                ...
                                <p className="text-sm text-slate-700 mb-3"><strong>Transaction ID:</strong> {payment.transactionId}</p>
                                <Link href={`/reciept/${payment.id}`} className="bg-blue-600 text-white border-none px-3 py-2 rounded text-sm transition-colors duration-200 hover:bg-blue-800 inline-block text-center w-full">
                                    Print
                                </Link>
                            </div>
                        ))}
